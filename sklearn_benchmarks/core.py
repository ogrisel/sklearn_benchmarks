...
class BenchmarkRunner:
    ...
    def _run_one_size(
        ...
        data = gen_data(
            self.dataset_name, n_samples, n_features, **dataset_param_overrides
        )
        ...
        print(
            f"{algo.name} (n_samples={n_samples}, n_features={n_features})"
    ...
    def run(self, algo, param_overrides={}, dataset_param_overrides={}):
        ...
        for ns in self.bench_rows:
            for nf in self.bench_dims:
                results.append(
                    self._run_one_size(
                        algo, ns, nf, param_overrides, dataset_param_overrides
                    )
                )
        return results
